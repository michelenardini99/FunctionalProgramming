...
trait NotTwoConsecutive[T] extends Parser[T]:
  private[this] var lastValueEqual = false
  private[this] var lastValue = Optional.empty()
  abstract override def parse(t: T): Boolean =
    if ((lastValue equals Optional.empty()) || !(t equals lastValue.get())) then lastValue = Optional.of(t) else lastValueEqual = true
    super.parse(t)
  abstract override def end: Boolean = !lastValueEqual && super.end
